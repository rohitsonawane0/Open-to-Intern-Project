...
exports.getCollegeDetails = async function (req, res) {
  try {
    ...
    if (!req.query.collegeName)
      return res
        .status(400)
        .send({ status: false, msg: "Query cannot be empty" });
    ...
    let collegeId = await CollegeModel.find({
      name: name,
      isDeleted: false,
    ...
    let intern = await InternModel.find({
      collegeId: internData,
      isDeleted: false,
    }).select({ collegeId: 0, isDeleted: 0, __v: 0 });
    college = college[0].toObject();
    if (
      intern.length == 0 ||
      intern.length == null ||
      intern.length == undefined
    ) {
