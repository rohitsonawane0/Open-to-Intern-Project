...
const checkNumber = function (str) {
  ...
  if (
    ...
    return `Mobile number cannot start with ${str[0]}`;
...
exports.validationInter = async function (req, res, next) {
  try {
    ...
    if (!valid(name))
      return res
        .status(400)
        .send({ status: false, mgs: "name must be present" });
    if (!nRegex.test(name))
      return res
        .status(400)
        .send({ status: false, mgs: "name  contains invalid characters" });
    ...
    if (
      existCollege.length == 0 ||
      existCollege == null ||
      existCollege == undefined
    )
